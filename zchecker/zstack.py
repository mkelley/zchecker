...
COLOR_DEFAULT = {"R - i": 0.12, "g - R": 0.39}
...
class ZStack(ZChecker):
    ...
    def clean_missing(self):
        ...
        with self.db as con:
            for stackid, fn in rows:
                ...
                self.logger.error("{} was expected, but does not exist.".format(fn))
                con.execute(
                    """
        ...
        self.logger.info(
            "{} files verified, {} database rows removed".format(exists, count - exists)
        )

    def stack(
        self, scale_by, n_baseline, objects=None, restack=False, start=None, stop=None
    ):
        ...
        for n, stackid, fn, nightlyids, nightly, baseline in data:
            ...
            if self._check_target_paths(self.config["stack path"], fn) and not restack:

                self.logger.error(
                    "Stack file exists, but was not expected.  Deleted: {}".format(fn)
                )
            ...
            headers = [
                fits.getheader(os.path.join(self.config["cutout path"], f))
                for f in nightly
            ]
            ...
            hdu = fits.HDUList()
            primary_header = self._header(
                self.config["cutout path"], nightly[calibrated]
            )
            ...
            if len(hdu) > 1:
                ...
                # OK to save
                hdu.writeto(
                    os.path.join(self.config["stack path"], fn), overwrite=restack
                )
                ...
                if stackid:
                    ...
                    self.logger.error(
                        ("Unsuccessful stack {}, deleting previous data.").format(fn)
                    )
        ...
        for nightid, night, objid, desg, filt in obs_sets:
            ...
            if len(stackid) == 0:
                ...
                msg = (
                    ...
                    "has been violated by stackids {} and {}".format(
                        stackid, row["stackid"]
                    )
                )
            ...
            fn = (
                "{desg}/{desg}-{date}-{prepost}{rh:.3f}-{filt}" "-ztf-stack.fits"
            ).format(
                desg=desg2file(desg),
                date=night.replace("-", ""),
    ...
    def _header(self, path, files):
        ...
        h["OBSALT"] = 1706.0, "Observatory altitude (m)"
        h["IMGTYPE"] = "object", "Image type"
        h["NIMAGES"] = N, "Number of images in stack"
        h["EXPOSURE"] = (
            sum([_["EXPOSURE"] for _ in headers]),
        ...
        h["CLRCOEFF"] = mean_key(headers, "CLRCOEFF", "Mean color coefficient", float)
        ...
        wcs = WCS(fits.getheader(os.path.join(path, wcsfn), extname="SANGLE"))
        h.update(wcs.to_header())
        h["WCSORIGN"] = wcsfn

        h["DBPID"] = (
            ",".join([str(_["DBPID"]) for _ in headers]),
    ...
    def _weighted_median(self, stack, unc, axis=0):
        ...
        if stack.shape[axis] == 1:
            ...
            wm = a[np.arange(a.shape[0])[:, None], np.arange(a.shape[1]), i]
    ...
    def _combine(self, files, scale_by, rh0, delta0, path):
        ...
        stack = []
        stack_ref = []
        # loop over each image, saving the header of the first good file
        header0 = None
        file0 = None
        for f in sorted(files):
            ...
            with fits.open(fn) as hdu:
                ...
                with warnings.catch_warnings():
                    ...
                    mask = hdu["SANGLE"].data < DATA_FLOOR + ~np.isfinite(
                        hdu["SANGLE"].data
                    )
                ...
                magzp = h["MAGZP"] + h["CLRCOEFF"] * COLOR_DEFAULT[h["PCOLOR"]]
                im *= (
                    10 ** (-0.4 * (magzp - 25.0))
                    * (h["DELTA"] / delta0) ** k[0]
                    * (h["RH"] / rh0) ** k[1]
        ...
        if len(stack_ref) == 0:
            ...
            im = np.ma.median(im, 2).filled(np.nan)
            combined_ref = fits.ImageHDU(im, wcs0.to_header())
