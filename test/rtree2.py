...
Without curvature considerations

Initial testing:

7 min and 750 MB to generate an rtree from 5780205 quads.

Searched entire archive for all targets in 100 sec: 40,000 follow-up
fields (about 700 fewer than rtree.py).
...
def corners2limits(ra, dec):
    ...
    xyz = np.zeros((3, 2))
    xyz[:, 0] = np.min(corners, 0)
    xyz[:, 1] = np.max(corners, 0)
    return xyz


rtree = sqlite3.connect('/oort/msk/zdata/rtree2-test.db')
rtree.execute('''
...
with ZChecker() as z:
    if setup:
        ...
        '''):
            jd0 = row['obsjd']
            jd1 = jd0 + FRAMETIME
            ra = np.array([row['ra' + i] for i in '1234'])
            dec = np.array([row['dec' + i] for i in '1234'])
    ...
    for target in targets:
        ...
        print('Checking {} with {} ephemeris points'.format(
            desg, len(jd)))
        ra = np.radians(ra)
        dec = np.radians(dec)
        ...
        print('    ', len(followup),
              'quads for {} follow-up'.format(desg))

tab.write('rtree2-finds.txt', format='ascii.fixed_width_two_line')
